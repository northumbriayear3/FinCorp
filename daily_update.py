...
REAL_GOLD_FIX_VERSION = "REAL_GOLD_LEVEL_FEATURE_FIX_V8_PCT_SLOPE_2026_07_28"
...
def build_predictions_for_commodity(commodity: str) -> dict[int, dict]:
    ...
    for task in ["level", "movement"]:
        ...
        for _, row in task_rows.iterrows():
            ...
            X_latest, forecast_date, missing = latest_feature_row(cache[key], features, strict=(commodity == "GOLD" and task == "level"))
            if missing:
...
def main() -> None:
    ...
    # Final check: Gold level must never be posted if feature pipeline breaks again.
