...
"""
FINAL GOLD SAFETY VERSION - blocks negative/unrealistic Gold level forecasts before posting to SQL.
...
def gold_level_prediction_needs_safety(feature_frame: pd.DataFrame, missing: list[str], pred: float) -> tuple[bool, str]:
    ...
    if pred < 1000 or pred > 10000:
        return True, f"Gold level prediction outside display/market safety range: {pred:.2f}"
