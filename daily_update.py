...
REAL_GOLD_FIX_VERSION = "TEMP_PROTECT_GOLD_ROWS_CRUDE_NATGAS_ONLY_2026_07_28"
...
def add_unprefixed_gold_level_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for w in [5, 10, 20, 30, 40, 50, 60, 100, 200]:
        ...
        # The saved Gold level models expect rolling_return_5, rolling_return_10, etc.
        # The previous GitHub updater created rolling_return_5d instead, so those
        # required model features were missing and were silently zero-filled.
        df[f"rolling_return_{w}"] = c / c.shift(w) - 1
        # Keep the older *_d alias as well in case any future exported model uses it.
        df[f"rolling_return_{w}d"] = df[f"rolling_return_{w}"]
    ...
    for task in ["level", "movement"]:
        ...
        for _, row in task_rows.iterrows():
            ...
            if missing:
                ...
                if commodity == "GOLD" and task == "level":
                    raise RuntimeError(
                        f"Gold level t+{h} is missing required model features: {missing}. "
                        "This is a real feature-pipeline mismatch, so the updater stopped instead of posting bad Gold values."
                    )
...
def main() -> None:
    ...
    # Gold level rows have been restored from the real saved dissertation models
    # using the original processed Gold feature dataset. Do not let the live
    # GitHub feature reconstruction overwrite those Gold rows until the Gold
    # live feature pipeline is rebuilt and validated.
    # This keeps Crude Oil and Natural Gas daily updates active.
    for commodity in ["CRUDE", "NATGAS"]:
